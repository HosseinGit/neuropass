...
async function deleteChat(userId, chatIdToDelete) {
    ...
    try {
        ...
        await batch.commit(); console.log(`Messages for chat ${chatIdToDelete} deleted.`);
        const chatDocRef = doc(db, "users", userId, "chats", chatIdToDelete);
        await deleteDoc(chatDocRef); console.log(`Chat ${chatIdToDelete} deleted.`);
        ...
        if (chatError) { chatError.textContent = "Error deleting chat. Please try again."; chatError.classList.add('active-error'); }
    }
}
...
function loadUserChats(userId) {
    ...
    unsubscribeChatList = onSnapshot(q, async (snapshot) => {
        ...
        snapshot.forEach((docSnap, index) => {
            ...
            deleteBtn.addEventListener('click', (e) => { e.stopPropagation(); deleteChat(userId, docSnap.id); });
            li.appendChild(deleteBtn);
            if (docSnap.id === currentChatId) { li.classList.add('active-chat'); }
